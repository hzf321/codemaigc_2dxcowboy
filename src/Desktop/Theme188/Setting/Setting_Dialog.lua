...
function Setting_Dialog:showThemeSetting()
	local contrl_node = self.root.contrl_node
	if contrl_node then
        local scale = 1 / SHRINKSCALE_V
        local size  = cc.size(169, 142)
        local bg    = contrl_node.bg
        if bg then
            size = bg:getContentSize()
        end

        local pos = bole.getNodePos(self.root, self.initPos)
        pos = cc.p(pos.x - size.width + 30 * scale, pos.y - size.height - 30 * scale)
		contrl_node:setVisible(true)
		contrl_node:stopAllActions()
		contrl_node:setOpacity(0)
		contrl_node:setScale(0)
        contrl_node:setPosition(pos)
		contrl_node:runAction(cc.Spawn:create(cc.FadeIn:create(0.2), cc.ScaleTo:create(0.2, scale)))
	end
end
...
function Setting_Dialog:onBtnServiceClicked()
    bole.playSounds("click")
    local url = "https://sites.google.com/view/zeus-fortune-terms-of-service"
...
function Setting_Dialog:onBtnPrivacyClicked()
    bole.playSounds("click")
    local url = "https://sites.google.com/view/zeusfortuneprivacypolicy"
	cc.Application:getInstance():openURL(url)
