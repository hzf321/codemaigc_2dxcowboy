...
function Footer_Node:ctor(theCtl, isPortrait)
    ...
    self.csb        = string.format("ui/header_footer/%s", self.isPortrait and "theme_footer" or "lobby_footer")
    ...
    if warehouse_node then
        for _, btn in pairs(warehouse_node:getChildren()) do
            ...
            local function onShowTip()
                ...
                elseif num == 8 then
                    HttpManager:getInstance():doReport(ReportConfig.btn_bottom_8_lobby)
                else 
                    HttpManager:getInstance():doReport(ReportConfig.btn_bottom_9_lobby)
                end
    ...
    if self.btnStop then
        ...
        local function stopBtnEvent(sender, eventType)
            if eventType == ccui.TouchEventType.began then
                ...
                self.btnStop.stop_fast:setPositionY(self.btnStop.stopPoY - 5)
            ...
            elseif eventType == ccui.TouchEventType.moved then
                ...
                if point.x < 0 or point.x > self.btnStop:getContentSize().width or point.y < 0 or point.y > self.btnStop:getContentSize().height then
                    ...
                    self.btnStop.stop_fast:setPositionY(self.btnStop.stopPoY)
