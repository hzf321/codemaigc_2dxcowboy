...
ThemeMysteriousPixies_JpView = class("ThemeMysteriousPixies_JpView")
local cls = ThemeMysteriousPixies_JpView
...
function cls:ctor(vCtl, nodeList)
	...
	self.jackpotDownParent = nodeList[1]
	self.jackpotTopParent = nodeList[2]
	self.jackpotTipParent = nodeList[3]
	self.jackpotNode = self.jackpotDownParent:getChildByName("jp_root")
...
function cls:_initLayout()
	if self.jackpotNode then
		...
		bole.changeParent(self.jackpotTipslock, self.jackpotTipParent, 10)
		bole.changeParent(self.jackpotTipsUnlock, self.jackpotTipParent, 10)
...
function cls:changeJackpotLockShow( jackpotType, showUnlock )
	if showUnlock then 
		...
		local animName = string.format("animation%s_2", jackpotType + 1)
...
function cls:playChangeJakcpotStateTip(_showType, unlockLevel)
	...
	if bole.isValidNode(showNode) and self.jackpotLockNodes[_type] and bole.isValidNode(self.jackpotLockNodes[_type].tipNode) then 
		...
		local posW = bole.getWorldPos(self.jackpotLockNodes[_type].tipNode)
		local posN = bole.getNodePos(self.jackpotTopParent, posW)
		showNode:setPosition(posN)
...
-----
    ...
    self.vCtl:playMusicByName(string.format("d_jp%s_show",(winJPType+1)))
...
function cls:playWinJpAnim( winType )
	...
	local animName = "animation"..(winType + 1) -- winType 0开始
	if (not self.jpWinAnimList or not bole.isValidNode(self.jpWinAnimList[winType])) and animName then 
		
		local data = {}
		data.file = self.vCtl:getSpineFile("jp_win")
		data.parent = self.jpAnimNode
		data.animateName = animName
		data.pos = cc.p(self.jackpotNormalNodes[winType]:getPosition())
		data.isLoop = true
		local _, jpAnim = bole.addAnimationSimple(data)
...
function cls:changeJPZorder( zType )
	if zType == 1 then 
		bole.changeParent(self.jackpotNode, self.jackpotTopParent)
	else
		bole.changeParent(self.jackpotNode, self.jackpotDownParent)
	end
end
-------------------------------------------------------
