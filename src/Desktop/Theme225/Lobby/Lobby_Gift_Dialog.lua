    ...
    self:runAction(
        cc.Sequence:create(
            ...
            cc.CallFunc:create(function ()
                ...
                local _, spine_down = bole.addSpineAnimation(self.root, nil, getSpinePath("spine_down"), cc.p(0,0), "appear", nil, nil, nil, true, false, nil)
                spine_down:registerSpineEventHandler( function(event)
                    if event.animation == "appear" then
                        spine_down:setAnimation(0, "loop", true) 
                    end
                end, sp.EventType.ANIMATION_COMPLETE)
