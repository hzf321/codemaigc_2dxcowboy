...
function Lobby_Theme:initPos()
	local posX = 150 + (self.index * 290)
	self:setPosition(self.is_lock and cc.p(posX, 360) or cc.p(400, 360))
end
...
function Lobby_Theme:updateLogo()
	...
	if theme_logo then
		...
		if  self.index == 10 then
			theme_logo:setScaleX(1)
			local logoX = theme_logo:getPositionX()
			theme_logo:setPositionX(logoX + 50)
		end
	end
end 
