...
local ThemeDialog = class("TCCommonDialog", CCSNode)
    ...
    for key, item in pairs(config) do
        ...
        elseif key == "btn" then
            ...
            if btnNode then
                ...
                local _, s = bole.addSpineAnimation(btnNode, nil, self.themeCtr:getSpineFile(item.name), cc.p(size.width / 2, size.height / 2), aniName, nil, nil, nil, true, true)
                if scalebtn then
                    s:setScale(scalebtn)
                end
            end
            ...
            if childNode then
                ...
                elseif item.startAction then
                    ...
                    local _, s = bole.addSpineAnimation(myParent, item.zorder, self.themeCtr:getSpineFile(fileName), cc.p(x, y), ani_name, nil, nil, nil, true, isLoop)
                    s:setName("spine")
